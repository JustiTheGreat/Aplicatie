...
public class AdaugaAnimal {
    ...
    public void set(Stage stage, String shop_username,ArrayList<AllProducts> all_products, ListView<String> lv, RadioButton fem, RadioButton masc, RadioButton pisici, RadioButton caini, RadioButton papagali, RadioButton pestisori, RadioButton hamsteri, RadioButton az, RadioButton dn,int az_state,int dn_state) {
        ...
        tip.getItems().addAll("PISICI","CAINI","PAPAGALI","PESTISORI","HAMSTERI");
        if (az.isSelected())
        {
    ...
    public void addLV () {
        try {
            ...
            for (AllProducts p : all_products)
                if (p.getObject().equals("ANIMAL") && ((Animal) p).getCentru().equals(shop_username) && ((Animal) p).getNumeRasa().equals(nume.getText()))
                    ok = 0;
            if (ok == 1 && isInt(data.getText()) && Integer.parseInt(data.getText()) >= 0 && !nume.getText().isEmpty() && !data.getText().isEmpty() && sex != null && tip!=null) {
                all_products.add(new Animal(nume.getText(),tip.getValue().toString(), sex.getValue().toString(), Integer.parseInt(data.getText()), shop_username, "DISPONIBIL"));
                write();
                filt_and_sort();
                stage.close();
            } else {
