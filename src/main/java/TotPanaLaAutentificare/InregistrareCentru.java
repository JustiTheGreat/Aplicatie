...
public class InregistrareCentru extends Utillity {
    @FXML
    private TextField nume_centru;
    @FXML
    ...
    public void inregistrare(javafx.event.ActionEvent actionEvent) {
        if (nume_centru.getText().trim().isEmpty() || judet.getText().trim().isEmpty() || localitate.getText().trim().isEmpty()
        ...
        else if (nume_centru.getText().trim().split(" ").length > 1 || judet.getText().trim().split(" ").length > 1 || localitate.getText().trim().split(" ").length > 1
            ...
            try {
                ...
                if (json_resp.equals("[]")) {
                    ...
                    if (json_resp.equals("[]")) {
                        ...
                        connection.disconnect();
                        if (json_resp.equals("[]")) {
                            parameters = "?username=" + username.getText().trim() + "&parola=" + parola.getText().trim() + "&nume=" + nume_centru.getText().trim()
                                    + "&judet=" + judet.getText().trim() + "&localitate=" + localitate.getText().trim()
                                    + "&strada=" + strada.getText().trim() + "&numar=" + numar.getText().trim() + "&telefon=" + telefon.getText().trim() + "&categorie=centru";
                            url = MyURL.getURL("/add-centru" + parameters);
                            connection = (HttpURLConnection) url.openConnection();
                            connection.setDoOutput(true);
                            connection.setRequestMethod("POST");
                            connection.disconnect();
