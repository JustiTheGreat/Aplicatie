...
public class InregistrarePetshop extends Utillity {
    @FXML
    private TextField nume_petshop;
    @FXML
    ...
    public void inregistrare(javafx.event.ActionEvent actionEvent) {
        if(nume_petshop.getText().trim().isEmpty()|| judet.getText().trim().isEmpty()|| localitate.getText().trim().isEmpty()
                || strada.getText().trim().isEmpty()|| numar.getText().trim().isEmpty()|| telefon.getText().trim().isEmpty()
                || username.getText().trim().isEmpty()||parola.getText().trim().isEmpty()
        ) message("Eroare!","Completati toate campurile");
        else if(nume_petshop.getText().trim().split(" ").length>1 || judet.getText().trim().split(" ").length>1 || localitate.getText().trim().split(" ").length>1
                || strada.getText().trim().split(" ").length>1 || numar.getText().trim().split(" ").length>1 || telefon.getText().trim().split(" ").length>1
                || username.getText().trim().split(" ").length>1 || parola.getText().trim().split(" ").length>1
            ...
            try {
                ...
                if (json_resp.equals("[]")) {
                    ...
                    if (json_resp.equals("[]")) {
                        ...
                        connection.disconnect();
                        if (json_resp.equals("[]")) {
                            parameters = "?username=" + username.getText().trim() + "&parola=" + parola.getText().trim() + "&nume=" + nume_petshop.getText().trim()
                                    + "&judet=" + judet.getText().trim() + "&localitate=" + localitate.getText().trim()
                                    + "&strada=" + strada.getText().trim() + "&numar=" + numar.getText().trim() + "&telefon=" + telefon.getText().trim() + "&categorie=petshop";
                            url = MyURL.getURL("/add-petshop" + parameters);
                            connection = (HttpURLConnection) url.openConnection();
                            connection.setDoOutput(true);
                            connection.setRequestMethod("POST");
                            connection.disconnect();
